...
getHikesListWithFilters = async (getMap,lengthMin, lengthMax, expectedTimeMin, expectedTimeMax, ascentMin, ascentMax, difficulty,centerlat,centerlon,latdeg,londegr) => new Promise((resolve, reject) => {
    ...
    db.all(sql, [lenMin, lenMax, expMin, expMax, ascMin, ascMax, centerlat, latdeg, centerlon, londegr, diff], (err, row) => {
        if(err) {
            reject(err);
            return;
        }
        ...
        else resolve(row.map((h) => ({id: h.IDHike, name: h.Name, author:h.Author,length: h.Length, expectedTime: h.ExpectedTime, ascent: h.Ascent, difficulty: h.Difficulty, startPoint: h.StartPoint, endPoint: h.EndPoint, description: h.Description})));
    });
...
const hikes = { getHikesList,getHikesListWithFilters,newHike,linkHutParkingToHike};
