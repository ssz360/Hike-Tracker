...
const newHike=async (name,author,desc,difficulty,file)=>{
    try {
        ...
        await hikesdao.newHike(name,author,len,ascent,desc,difficulty.toUpperCase(),JSON.stringify(coors[0]),JSON.stringify(coors[coors.length-1]),"",JSON.stringify(coors),center,Math.max(...lats),Math.max(...lons),Math.min(...lats),Math.min(...lons));
    } catch (error) {
